stat_cast <- R6::R6Class(
  ...
  public = list(
    ...
    clean_data = function() {
      ...
      if (param_get(self$url, 'player_type') == 'hitter') {
        data <- data[, names(data) %in% private$hit_fields]
      }
    ...
    compile_mix_summary = function(player){
      data <- self$mix[[player]] %>% mutate(month = floor_date( unit = 'month',x= as.Date(game_date))) %>%
    ...
    create_avg_pitch_chart = function(player, values, filter_pitch, save_plot = FALSE) {
      ...
      data <-
        self$pitch_timeseries_group(player, values, filter_pitch)
    ...
    create_pitch_scatter = function(player,
                                    filter_pitch = NULL,
                                    save_plot = FALSE) {
      if (!is.null(filter_pitch)) {
        title <-
          ggtitle(paste(player, ' ', filter_pitch, ':', 'Pitch scatter'))
    ...
    show_pitch_mix = function(player,
                              save_plot = TRUE,
                              min = NULL,
                              max = NULL) {
      mix <- self$compile_pitch_mix(player)
      mix_group <- self$compile_mix_summary(player)
      
      title <- ggtitle(paste(player, 'Pitch Mix', setp = ' '))
      
      
      plot <-
        ggplot(data=mix_group,
